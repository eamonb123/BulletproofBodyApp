...
// Restaurants excluded from client view (no verified data)
const CLIENT_EXCLUDED_RESTAURANTS = new Set(["sushi", "indian", "chinese_takeout"]);

// Food type keywords → restaurant IDs for semantic search
// When someone searches "sandwich", show all sandwich spots
const FOOD_TYPE_KEYWORDS: Record<string, string[]> = {
  sandwich: ["subway", "jerseymikes", "jimmyjohns", "firehousesubs", "panera", "wawa", "sheetz", "wholefoods"],
  sub: ["subway", "jerseymikes", "jimmyjohns", "firehousesubs"],
  burger: ["mcdonalds", "burgerking", "wendys", "innoutburger", "shakeshack", "whataburger", "sonicdrivein", "dairyqueen"],
  pizza: ["pizzahut", "papajohns", "littlecaesars", "dominos"],
  chicken: ["chickfila", "raisingcanes", "zaxbys", "popeyes", "kfc", "wholefoods"],
  wings: ["bww", "wingstop", "zaxbys"],
  mexican: ["chipotle", "tacobell", "qdoba", "moessouthwestgrill"],
  burrito: ["chipotle", "tacobell", "qdoba", "moessouthwestgrill"],
  bowl: ["chipotle", "cava", "sweetgreen", "pandaexpress", "wholefoods"],
  salad: ["sweetgreen", "panera", "cava", "wholefoods"],
  chinese: ["pandaexpress"],
  asian: ["pandaexpress"],
  smoothie: ["smoothieking", "tropicalsmoothiecafe"],
  breakfast: ["dunkin", "dennys", "ihop", "firstwatch", "wafflehouse", "crackerbarrel"],
  coffee: ["dunkin", "starbucks"],
  steak: ["longhornsteakhouse", "outbacksteakhouse", "texasroadhouse"],
  seafood: ["redlobster"],
  pasta: ["olivegarden"],
  italian: ["olivegarden"],
  ribs: ["applebees", "chilis", "outbacksteakhouse", "texasroadhouse"],
  bbq: ["applebees", "chilis"],
  poke: ["cava", "sweetgreen"],
  healthy: ["sweetgreen", "cava", "firstwatch", "tropicalsmoothiecafe", "wholefoods"],
  grocery: ["wholefoods"],
  "family meal": ["wholefoods"],
  salmon: ["wholefoods"],
  "meal prep": ["wholefoods"],
  "whole foods": ["wholefoods"],
  fast: ["mcdonalds", "burgerking", "wendys", "tacobell", "chickfila"],
  drive: ["mcdonalds", "burgerking", "wendys", "chickfila", "innoutburger", "sonicdrivein"],
};
...
function HeroScreen({ restaurants, onSelectRestaurant }: { restaurants: Restaurant[]; onSelectRestaurant: (id: string) => void }) {
  ...
  const heroFiltered = useMemo(() => {
    ...
    // Direct name/cuisine match
    const directMatches = restaurants.filter(
      (r) => r.name.toLowerCase().includes(q) || r.cuisine?.toLowerCase().includes(q) || r.food_keywords?.toLowerCase().includes(q)
    );
    if (directMatches.length > 0) return directMatches;

    // Food type keyword match — "sandwich" finds Subway, Jimmy John's, etc.
    const keywordIds = new Set<string>();
    for (const [keyword, ids] of Object.entries(FOOD_TYPE_KEYWORDS)) {
      if (keyword.includes(q) || q.includes(keyword)) {
        for (const id of ids) keywordIds.add(id);
      }
    }
    if (keywordIds.size > 0) {
      return restaurants.filter((r) => keywordIds.has(r.id));
    }
...
function BiblePage() {
  ...
  const initialRestaurant = searchParams.get("r");

  const [restaurants, setRestaurants] = useState<Restaurant[]>([]);
  const [selectedId, setSelectedId] = useState<string | null>(initialRestaurant);
  ...
  const [flow, setFlow] = useState<"hero" | "grid" | "detail" | "gate" | "free">("free");
  const [completedSwaps, setCompletedSwaps] = useState<CompletedSwap[]>([]);
  const [showTutorial, setShowTutorial] = useState(false);
  const [passedGate, setPassedGate] = useState(true);
  ...
  useEffect(() => {
    fetch("/api/restaurants")
      ...
      .then((data) => {
        setRestaurants(
          (data.restaurants as Restaurant[]).filter(
            (r: Restaurant) => !CLIENT_EXCLUDED_RESTAURANTS.has(r.id)
          )
        );
        setLoading(false);
  ...
  const filteredRestaurants = useMemo(() => {
    ...
    // Direct name/cuisine match first
    const directMatches = restaurants.filter(
      (r) =>
  ...
  return (
    <div>
      ...
      <div className="relative h-6 flex items-center">
        {/* Invisible native range inputs — full width for hit area */}
        <input
          type="range"
          min={min}
          max={max}
          ...
          className="absolute w-full appearance-none bg-transparent pointer-events-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:shadow-[0_0_6px_rgba(0,0,0,0.4)] [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:cursor-grab [&::-webkit-slider-thumb]:active:cursor-grabbing [&::-webkit-slider-thumb]:relative [&::-webkit-slider-thumb]:z-20"
        />
        <input
          type="range"
          min={min}
          ...
          className="absolute w-full appearance-none bg-transparent pointer-events-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:shadow-[0_0_6px_rgba(0,0,0,0.4)] [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:cursor-grab [&::-webkit-slider-thumb]:active:cursor-grabbing [&::-webkit-slider-thumb]:relative [&::-webkit-slider-thumb]:z-30"
        />
        {/* Visual track — inset to match native thumb travel range (half thumb = 10px) */}
        <div className="absolute left-[10px] right-[10px] h-1.5 rounded-full bg-zinc-700" />
        <div
          className="absolute h-1.5 rounded-full bg-emerald-500"
          style={{
            left: `calc(10px + (100% - 20px) * ${pctLo / 100})`,
            right: `calc(10px + (100% - 20px) * ${(100 - pctHi) / 100})`,
          }}
        />
      </div>
    </div>
...
// ─── Collapsible order script rationale ──────────────
function OrderScriptDetails({ rationalePart, mealQuery }: { rationalePart: string; mealQuery: string }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="mt-2 border-t border-emerald-500/15 pt-2">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-emerald-500/70 hover:text-emerald-400 transition-colors"
      >
        <svg className={`h-3 w-3 transition-transform ${open ? "rotate-90" : ""}`} fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
        </svg>
        Why this works
      </button>
      {open && (
        <p className="mt-2 text-xs leading-relaxed text-zinc-400"><HighlightText text={rationalePart} query={mealQuery} /></p>
      )}
    </div>
  );
}
...
// ─── Client Swap View (simple reference cards) ──────
function ClientSwapView({
  restaurant,
  detail,
  loading,
  onBack,
  ...
  // Only show Eamon-approved swaps
  const allSwaps = detail.meals.filter((m) => m.is_swap === 1 && m.source === "eamon");

  // Compute ranges from data — round up to nearest step so slider max is always reachable
  const calStep = 10;
  const proStep = 5;
  const maxCalInData = Math.ceil(Math.max(...allSwaps.map((s) => Math.round(s.totals.calories)), 100) / calStep) * calStep;
  const maxProteinInData = Math.ceil(Math.max(...allSwaps.map((s) => Math.round(s.totals.protein)), 10) / proStep) * proStep;

  const [calRange, setCalRange] = useState<[number, number]>([0, maxCalInData]);
  const [proRange, setProRange] = useState<[number, number]>([0, maxProteinInData]);
  const [mealSearch, setMealSearch] = useState("");

  // Sync slider ranges when data arrives/changes (useState initial value only applies on first render)
  const prevMaxCal = useRef(maxCalInData);
  const prevMaxPro = useRef(maxProteinInData);
  useEffect(() => {
    if (maxCalInData !== prevMaxCal.current) {
      setCalRange(([lo]) => [lo, maxCalInData]);
      prevMaxCal.current = maxCalInData;
    }
    if (maxProteinInData !== prevMaxPro.current) {
      setProRange(([lo]) => [lo, maxProteinInData]);
      prevMaxPro.current = maxProteinInData;
    }
  }, [maxCalInData, maxProteinInData]);

  // Filter swaps by search query, then by macro ranges
  const mealQuery = mealSearch.toLowerCase().trim();
  const searchedSwaps = mealQuery
    ? allSwaps.filter((s) =>
        s.name.toLowerCase().includes(mealQuery) ||
        s.ingredients.some((ing) => ing.name.toLowerCase().includes(mealQuery)) ||
        (s.order_script && s.order_script.toLowerCase().includes(mealQuery)) ||
        (s.description && s.description.toLowerCase().includes(mealQuery))
      )
    : allSwaps;

  const swaps = searchedSwaps.filter((s) => {
    const cal = Math.round(s.totals.calories);
    const pro = Math.round(s.totals.protein);
    return cal >= calRange[0] && cal <= calRange[1] && pro >= proRange[0] && pro <= proRange[1];
  });

  const isFiltered = calRange[0] > 0 || calRange[1] < maxCalInData || proRange[0] > 0 || proRange[1] < maxProteinInData || mealQuery.length > 0;
  ...
  return (
    <div>
      ...
      {/* Restaurant header */}
      <div className="flex items-center gap-5 px-6 pt-4 pb-6">
        <div className="flex-shrink-0">
          <RestaurantLogo id={restaurant.id} name={restaurant.name} className="h-16 w-16 rounded-2xl" />
        </div>
        <div>
          <h2 className="text-3xl font-bold text-white">{restaurant.name}</h2>
          <p className="mt-1 text-base text-zinc-400">{swaps.length} of {allSwaps.length} option{allSwaps.length !== 1 ? "s" : ""}</p>
        </div>
      </div>
      ...
      <div className="mx-6 mb-4">
        <div className="relative">
          <svg className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-zinc-500" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
          </svg>
          <input
            type="text"
            ...
            placeholder="Search meals or ingredients... (e.g. grilled, fried, wrap)"
            className="w-full rounded-xl border border-zinc-800 bg-zinc-900/60 py-3 pl-11 pr-10 text-sm text-white placeholder-zinc-500 outline-none transition-colors focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20"
          />
          {mealSearch && (
            <button
              onClick={() => setMealSearch("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-1 text-zinc-500 hover:text-white transition-colors"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
              </svg>
      ...
      {/* Filter sliders — dual range for calories + protein */}
      <div className="mx-6 mb-6 rounded-2xl border border-zinc-800 bg-zinc-900/60 px-5 py-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <DualRangeSlider
            ...
            unit="g"
            min={0}
            max={maxProteinInData}
            step={proStep}
            value={proRange}
            onChange={setProRange}
          />
        </div>
        {isFiltered && (
          <button
            onClick={() => { setCalRange([0, maxCalInData]); setProRange([0, maxProteinInData]); setMealSearch(""); }}
      ...
      {/* Swap cards — 2-up grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 px-6 pb-24">
        {swaps.length === 0 && (
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-10 text-center">
            <p className="text-lg text-zinc-400">No options yet for this restaurant.</p>
          </div>
        )}
        {swaps.map((swap) => {
          const swapCal = Math.round(swap.totals.calories);
          const swapProtein = Math.round(swap.totals.protein);
          const ings = swap.ingredients.map((i) => i as MealIngredient);
          ...
          return (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3 }}
              className="rounded-2xl border border-emerald-500/20 bg-gradient-to-b from-zinc-900 to-zinc-950 overflow-hidden flex flex-col"
            >
              {/* Meal image */}
              {swap.sprite_url ? (
                ...
                <div className="aspect-[16/9] w-full bg-zinc-900 flex items-center justify-center">
                  <RestaurantLogo id={restaurant.id} name={restaurant.name} className="h-20 w-20 rounded-2xl opacity-30" />
                </div>
              )}

              {/* Meal name */}
              <div className="px-5 pt-5 pb-1">
                <h3 className="text-xl font-extrabold text-white leading-snug"><HighlightText text={swap.name} query={mealQuery} /></h3>
              </div>

              {/* Macros bar — single unified line */}
              <div className="flex items-baseline gap-1.5 px-5 pb-4">
                <span className="text-3xl font-black text-emerald-400">{swapCal}</span>
                <span className="text-sm font-medium text-emerald-400/80">cal</span>
                <span className="mx-1.5 text-zinc-600">·</span>
                <span className="text-3xl font-black text-white">{swapProtein}g</span>
                <span className="text-sm font-medium text-zinc-400">protein</span>
              </div>

              {/* Ingredient breakdown */}
              <div className="mx-5 mb-4 rounded-xl bg-zinc-800/40 overflow-hidden">
                {ings.map((ing, i) => {
                  const cal = Math.round(ing.calories * ing.quantity);
                  const pro = Math.round(ing.protein_g * ing.quantity);
                  const qty = ing.quantity > 1 ? ` (x${ing.quantity})` : ing.quantity < 1 ? " (half)" : "";
                  return (
                    <div key={i} className={`flex items-center justify-between px-3 py-2.5 ${i > 0 ? "border-t border-zinc-700/30" : ""}`}>
                      <span className="text-sm text-zinc-300"><HighlightText text={ing.name + qty} query={mealQuery} /></span>
                      <div className="flex items-center gap-3 flex-shrink-0">
                        <span className="text-sm font-semibold text-emerald-400">{cal}</span>
                        <span className="text-xs text-zinc-500">{pro}g P</span>
                      </div>
                    </div>
                  );
                })}
              ...
              {/* Order script — split into say-this vs rationale */}
              {swap.order_script && (() => {
                const script = swap.order_script;
                const headsUpIdx = script.search(/HEADS UP:|heads up:/i);
                const sayPart = headsUpIdx > -1 ? script.slice(0, headsUpIdx).trim() : script;
                const rationalePart = headsUpIdx > -1 ? script.slice(headsUpIdx).trim() : null;

                return (
                  <div className="mx-5 mb-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-4 py-3">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-500 mb-1">Say this at the counter</p>
                    <p className="text-sm leading-relaxed text-zinc-200"><HighlightText text={sayPart} query={mealQuery} /></p>
                    {rationalePart && (
                      <OrderScriptDetails rationalePart={rationalePart} mealQuery={mealQuery} />
                    )}
                  </div>
                );
              })()}
              ...
              {/* Eamon's pick badge */}
              <div className="flex items-center justify-center gap-2 py-3 mt-auto border-t border-zinc-800/40">
                <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-500/60">Eamon&apos;s Pick</span>
                <svg className="h-3.5 w-3.5 text-emerald-500/60" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
              </div>
            </motion.div>
          );
        })}
...
function SwapSavingsCard(props: any) { return <div {...props} />; }
